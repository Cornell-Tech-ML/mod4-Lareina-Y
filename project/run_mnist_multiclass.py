...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        ...
        ln1 = self.linear1(avgpool.view(BATCH, 392)).relu()
        ln1 = minitorch.dropout(ln1, 0.25, not self.training)
        ln2 = self.linear2(ln1)
        return minitorch.logsoftmax(ln2, 1)
...
class ImageTrain:
    ...
    def train(
        self, data_train, data_val, learning_rate, max_epochs=60, log_fn=default_log_fn
    ):
